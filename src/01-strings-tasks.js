...
function extractNameFromTemplate(value) {
  ...
  const arr = newVal.split(' ').slice(1);
  return arr.join(' ');
}
...
function getRectangleString(width, height) {
  ...
  result += `${border.topLeft}${border.hor.repeat(width - 2)}${border.topRight}\n`;
  result += `${border.ver}${' '.repeat(width - 2)}${border.ver}\n`.repeat(height - 2);
  result += `${border.bottomLeft}${border.hor.repeat(width - 2)}${border.bottomRight}\n`;
  return result;
}
...
function encodeToRot13(str) {
  ...
  return str
    .split('').map((el) => {
      const index = from.indexOf(el);
      return (index !== -1) ? to[index] : el;
    }).join('');
}
